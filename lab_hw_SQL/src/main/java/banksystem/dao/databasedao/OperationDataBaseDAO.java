...
public class OperationDataBaseDAO implements OperationDAO {
    ...
    @Override
    public Operation getOperationByID(int loanID) {
    ...
    public void updateOperation(@NotNull Operation operation) {
        try (PreparedStatement ps = MySQL.getConnection().prepareStatement(UPDATE_OPERATION)) {
            ...
            ps.setDouble(2, operation.getPercent());
            ps.setString(3,operation.getCurrency().toString());
            ps.setDate(4,operation.getDateIssue());
            ps.setDate(5,operation.getDateReturn());
            ps.addBatch(SET_FOREIGN_KEY_CHECKS+"1");
            ps.executeUpdate();
            ps.executeBatch();
        } catch (SQLException e) {
