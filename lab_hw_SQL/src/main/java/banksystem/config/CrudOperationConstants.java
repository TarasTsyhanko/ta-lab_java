...
public class CrudOperationConstants {
    ...
    public static final String SELECT_ALL_OPERATION_BY_CLIENT = "SELECT distinct IDOperation,d.BankName,IDClient,Amount,Percent,Currency,Date_Issue,Date_Return,TypeOperation FROM Operation d JOIN Person p, Bank b WHERE p.IDPerson=d.IDClient AND p.IDPerson=? OR b.IDBank=d.IDClient AND b.IDBank=?";
    public static final String SELECT_ALL_OPERATION_IN_ONE_BANK = "SELECT IDOperation,d.BankName,IDClient,Amount,Percent,Currency,Date_Issue,Date_Return,TypeOperation FROM Operation d JOIN Bank b WHERE b.BankName=?";
    public static final String SELECT_OPERATION_BY_ID="SELECT * FROM Loan WHERE IDLoan=?";
    ...
    public static final String UPDATE_OPERATION= "UPDATE Operation SET Amount=?,Percent=?,Currency=?,Date_Issue=?,Date_Return=? WHERE IDOperation=?";
