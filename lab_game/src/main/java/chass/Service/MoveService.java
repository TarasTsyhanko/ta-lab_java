...
public class MoveService {
    ...
    private void chooseNextPisotion(int hor1, int vert1) {
        ...
        while (true) {
            ...
            if (isPosition(s)) {
                ...
                try {
                    if (board.getFigura(hor1,vert1).coler.equals(board.getFigura(hor2,vert2).coler)) {
    ...
    private boolean turnWhiteOrBlack(boolean white, String pos){
        if (white && board.getFigura(board.getHoryxontal(pos),board.getVertycal(pos)).coler.equals(Coler.WHITE)){
            whiteMove = false;
            return true;
        }else if (!white&& board.getFigura(board.getHoryxontal(pos),board.getVertycal(pos)).coler.equals(Coler.BLACK)){
            whiteMove = true;
            return true;
        }
        return false;
